...
void AVRBasePlayer::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("AimAction", IE_Pressed, this, &AVRBasePlayer::AimPressedAction);
	PlayerInputComponent->BindAction("AimAction", IE_Released, this, &AVRBasePlayer::AimReleasedAction);
	PlayerInputComponent->BindAction("CameraAction", IE_Pressed, this, &AVRBasePlayer::CameraPressedAction);
	PlayerInputComponent->BindAction("CameraAction", IE_Released, this, &AVRBasePlayer::CameraReleasedAction);
}
...
void AVRBasePlayer::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME(AVRBasePlayer, TargetRagdollLocation);
	DOREPLIFETIME_CONDITION(AVRBasePlayer, ReplicatedCurrentAcceleration, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AVRBasePlayer, ReplicatedControlRotation, COND_SkipOwner);

	DOREPLIFETIME(AVRBasePlayer, DesiredGait);
	DOREPLIFETIME_CONDITION(AVRBasePlayer, DesiredStance, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AVRBasePlayer, DesiredRotationMode, COND_SkipOwner);

	DOREPLIFETIME_CONDITION(AVRBasePlayer, RotationMode, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AVRBasePlayer, OverlayState, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AVRBasePlayer, ViewMode, COND_SkipOwner);
	DOREPLIFETIME_CONDITION(AVRBasePlayer, VisibleMesh, COND_SkipOwner);
}

void AVRBasePlayer::OnBreakfall_Implementation()
{
	Replicated_PlayMontage(GetRollAnimation(), 1.35);
}

void AVRBasePlayer::Replicated_PlayMontage_Implementation(UAnimMontage* Montage, float PlayRate)
{
...
void AVRBasePlayer::PreInitializeComponents()
{
...
void AVRBasePlayer::SetAimYawRate(float NewAimYawRate)
{
...
void AVRBasePlayer::Tick(float DeltaTime)
{
...
void AVRBasePlayer::RagdollStart()
{
...
void AVRBasePlayer::Server_SetMeshLocationDuringRagdoll_Implementation(FVector MeshLocation)
{
	TargetRagdollLocation = MeshLocation;
}

void AVRBasePlayer::SetMovementState(const EALSMovementState NewState, bool bForce)
{
...
void AVRBasePlayer::SetMovementAction(const EALSMovementAction NewAction, bool bForce)
{
...
void AVRBasePlayer::SetStance(const EALSStance NewStance, bool bForce)
{
...
void AVRBasePlayer::SetGait(const EALSGait NewGait, bool bForce)
{
...
void AVRBasePlayer::SetDesiredStance(EALSStance NewStance)
{
...
void AVRBasePlayer::Server_SetDesiredStance_Implementation(EALSStance NewStance)
{
	SetDesiredStance(NewStance);
}

void AVRBasePlayer::SetDesiredGait(const EALSGait NewGait)
{
	DesiredGait = NewGait;
	if (GetLocalRole() == ROLE_AutonomousProxy)
	{
...
void AVRBasePlayer::Server_SetDesiredGait_Implementation(EALSGait NewGait)
{
	SetDesiredGait(NewGait);
}

void AVRBasePlayer::SetDesiredRotationMode(EALSRotationMode NewRotMode)
{
...
void AVRBasePlayer::Server_SetDesiredRotationMode_Implementation(EALSRotationMode NewRotMode)
{
	SetDesiredRotationMode(NewRotMode);
}

void AVRBasePlayer::SetRotationMode(const EALSRotationMode NewRotationMode, bool bForce)
{
...
void AVRBasePlayer::Server_SetRotationMode_Implementation(EALSRotationMode NewRotationMode, bool bForce)
{
	SetRotationMode(NewRotationMode, bForce);
}

void AVRBasePlayer::SetViewMode(const EALSViewMode NewViewMode, bool bForce)
{
...
void AVRBasePlayer::Server_SetViewMode_Implementation(EALSViewMode NewViewMode, bool bForce)
{
	SetViewMode(NewViewMode, bForce);
}

void AVRBasePlayer::SetOverlayState(const EALSOverlayState NewState, bool bForce)
{
...
void AVRBasePlayer::Server_SetOverlayState_Implementation(EALSOverlayState NewState, bool bForce)
{
	SetOverlayState(NewState, bForce);
}

void AVRBasePlayer::EventOnLanded()
{
	...
	else
	{
		...
		GetWorldTimerManager().SetTimer(OnLandedFrictionResetTimer, this,
		                                &AVRBasePlayer::OnLandFrictionReset, 0.5f, false);
	}
}

void AVRBasePlayer::Multicast_OnLanded_Implementation()
{
...
void AVRBasePlayer::EventOnJumped()
{
...
void AVRBasePlayer::Server_PlayMontage_Implementation(UAnimMontage* Montage, float PlayRate)
{
	if (MainAnimInstance)
	{
		MainAnimInstance->Montage_Play(Montage, PlayRate);
...
void AVRBasePlayer::Multicast_PlayMontage_Implementation(UAnimMontage* Montage, float PlayRate)
{
...
void AVRBasePlayer::Multicast_OnJumped_Implementation()
{
	if (!IsLocallyControlled())
	{
		EventOnJumped();
...
void AVRBasePlayer::Server_RagdollStart_Implementation()
{
	Multicast_RagdollStart();
}

void AVRBasePlayer::Multicast_RagdollStart_Implementation()
{
	RagdollStart();
}

void AVRBasePlayer::Server_RagdollEnd_Implementation(FVector CharacterLocation)
{
	Multicast_RagdollEnd(CharacterLocation);
}

void AVRBasePlayer::Multicast_RagdollEnd_Implementation(FVector CharacterLocation)
{
	RagdollEnd();
}

void AVRBasePlayer::SetActorLocationAndTargetRotation(FVector NewLocation, FRotator NewRotation)
{
	SetActorLocationAndRotation(NewLocation, NewRotation);
	TargetRotation = NewRotation;
}

void AVRBasePlayer::SetMovementModel()
{
...
void AVRBasePlayer::ForceUpdateCharacterState()
{
...
void AVRBasePlayer::SetHasMovementInput(bool bNewHasMovementInput)
{
...
FALSMovementSettings AVRBasePlayer::GetTargetMovementSettings() const
{
...
bool AVRBasePlayer::CanSprint() const
{
...
void AVRBasePlayer::SetIsMoving(bool bNewIsMoving)
{
...
FVector AVRBasePlayer::GetMovementInput() const
{
	return ReplicatedCurrentAcceleration;
}

void AVRBasePlayer::SetMovementInputAmount(float NewMovementInputAmount)
{
...
void AVRBasePlayer::SetSpeed(float NewSpeed)
{
	Speed = NewSpeed;
...
float AVRBasePlayer::GetAnimCurveValue(FName CurveName) const
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::SetVisibleMesh(USkeletalMesh* NewVisibleMesh)
{
...
void AVRBasePlayer::Server_SetVisibleMesh_Implementation(USkeletalMesh* NewVisibleMesh)
{
	SetVisibleMesh(NewVisibleMesh);
}

void AVRBasePlayer::SetRightShoulder(bool bNewRightShoulder)
{
...
ECollisionChannel AVRBasePlayer::GetThirdPersonTraceParams(FVector& TraceOrigin, float& TraceRadius)
{
...
FTransform AVRBasePlayer::GetThirdPersonPivotTarget()
{
	return GetActorTransform();
}

FVector AVRBasePlayer::GetFirstPersonCameraTarget()
{
	return GetMesh()->GetSocketLocation(NAME_FP_Camera);
}

void AVRBasePlayer::GetCameraParameters(float& TPFOVOut, float& FPFOVOut, bool& bRightShoulderOut) const
{
...
void AVRBasePlayer::SetAcceleration(const FVector& NewAcceleration)
{
...
void AVRBasePlayer::RagdollUpdate(float DeltaTime)
{
...
void AVRBasePlayer::SetActorLocationDuringRagdoll(float DeltaTime)
{
	if (IsLocallyControlled())
	{
...
void AVRBasePlayer::OnMovementModeChanged(EMovementMode PrevMovementMode, uint8 PreviousCustomMode)
{
...
void AVRBasePlayer::OnMovementStateChanged(const EALSMovementState PreviousState)
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::OnMovementActionChanged(const EALSMovementAction PreviousAction)
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::OnStanceChanged(const EALSStance PreviousStance)
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::OnRotationModeChanged(EALSRotationMode PreviousRotationMode)
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::OnGaitChanged(const EALSGait PreviousGait)
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::OnViewModeChanged(const EALSViewMode PreviousViewMode)
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::OnOverlayStateChanged(const EALSOverlayState PreviousState)
{
	if (MainAnimInstance)
	{
...
void AVRBasePlayer::OnVisibleMeshChanged(const USkeletalMesh* PrevVisibleMesh)
{
...
void AVRBasePlayer::OnStartCrouch(float HalfHeightAdjust, float ScaledHalfHeightAdjust)
{
...
void AVRBasePlayer::OnEndCrouch(float HalfHeightAdjust, float ScaledHalfHeightAdjust)
{
...
void AVRBasePlayer::OnJumped_Implementation()
{
...
void AVRBasePlayer::Landed(const FHitResult& Hit)
{
	Super::Landed(Hit);
...
void AVRBasePlayer::OnLandFrictionReset()
{
	// Reset the braking friction
	GetCharacterMovement()->BrakingFrictionFactor = 0.0f;
}

void AVRBasePlayer::SetEssentialValues(float DeltaTime)
{
...
void AVRBasePlayer::UpdateCharacterMovement()
{
...
void AVRBasePlayer::UpdateGroundedRotation(float DeltaTime)
{
	if (MovementAction == EALSMovementAction::None)
	{
...
void AVRBasePlayer::UpdateInAirRotation(float DeltaTime)
{
	if (RotationMode == EALSRotationMode::VelocityDirection || RotationMode == EALSRotationMode::LookingDirection)
	{
...
EALSGait AVRBasePlayer::GetAllowedGait() const
{
...
EALSGait AVRBasePlayer::GetActualGait(EALSGait AllowedGait) const
{
...
void AVRBasePlayer::SmoothCharacterRotation(FRotator Target, float TargetInterpSpeed, float ActorInterpSpeed,
                                                float DeltaTime)
{
...
float AVRBasePlayer::CalculateGroundedRotationRate() const
{
...
void AVRBasePlayer::LimitRotation(float AimYawMin, float AimYawMax, float InterpSpeed, float DeltaTime)
{
...
void AVRBasePlayer::PlayerForwardMovementInput(float Value)
{
...
void AVRBasePlayer::PlayerRightMovementInput(float Value)
{
	if (MovementState == EALSMovementState::Grounded || MovementState == EALSMovementState::InAir)
	{
		// Default camera relative movement behavior
...
void AVRBasePlayer::PlayerCameraUpInput(float Value)
{
	AddControllerPitchInput(LookUpDownRate * Value);
}

void AVRBasePlayer::PlayerCameraRightInput(float Value)
{
	AddControllerYawInput(LookLeftRightRate * Value);
}

void AVRBasePlayer::JumpPressedAction()
{
...
void AVRBasePlayer::JumpReleasedAction()
{
	StopJumping();
}

void AVRBasePlayer::SprintPressedAction()
{
	SetDesiredGait(EALSGait::Sprinting);
}

void AVRBasePlayer::SprintReleasedAction()
{
	SetDesiredGait(EALSGait::Running);
}

void AVRBasePlayer::AimPressedAction()
{
...
void AVRBasePlayer::AimReleasedAction()
{
	if (ViewMode == EALSViewMode::ThirdPerson)
	{
		SetRotationMode(DesiredRotationMode);
...
void AVRBasePlayer::CameraPressedAction()
{
	UWorld* World = GetWorld();
	check(World);
	CameraActionPressedTime = World->GetTimeSeconds();
	GetWorldTimerManager().SetTimer(OnCameraModeSwapTimer, this,
	                                &AVRBasePlayer::OnSwitchCameraMode, ViewModeSwitchHoldTime, false);
}

void AVRBasePlayer::CameraReleasedAction()
{
...
void AVRBasePlayer::OnSwitchCameraMode()
{
	// Switch camera mode
	if (ViewMode == EALSViewMode::FirstPerson)
	{
...
void AVRBasePlayer::StancePressedAction()
{
...
void AVRBasePlayer::WalkPressedAction()
{
...
void AVRBasePlayer::RagdollPressedAction()
{
...
void AVRBasePlayer::VelocityDirectionPressedAction()
{
...
void AVRBasePlayer::LookingDirectionPressedAction()
{
	SetDesiredRotationMode(EALSRotationMode::LookingDirection);
	SetRotationMode(EALSRotationMode::LookingDirection);
}

void AVRBasePlayer::ReplicatedRagdollStart()
{
	if (HasAuthority())
	{
		Multicast_RagdollStart();
...
void AVRBasePlayer::ReplicatedRagdollEnd()
{
	if (HasAuthority())
	{
...
void AVRBasePlayer::OnRep_RotationMode(EALSRotationMode PrevRotMode)
{
	OnRotationModeChanged(PrevRotMode);
}

void AVRBasePlayer::OnRep_ViewMode(EALSViewMode PrevViewMode)
{
	OnViewModeChanged(PrevViewMode);
}

void AVRBasePlayer::OnRep_OverlayState(EALSOverlayState PrevOverlayState)
{
	OnOverlayStateChanged(PrevOverlayState);
}

void AVRBasePlayer::OnRep_VisibleMesh(USkeletalMesh* NewVisibleMesh)
{
