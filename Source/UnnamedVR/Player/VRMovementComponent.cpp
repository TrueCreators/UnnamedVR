...
void UVRMovementComponent::OnMovementUpdated(float DeltaTime, const FVector& OldLocation,
...
void UVRMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
...
float UVRMovementComponent::GetMaxAcceleration() const
{
...
float UVRMovementComponent::GetMaxBrakingDeceleration() const
{
...
void UVRMovementComponent::UpdateFromCompressedFlags(uint8 Flags) // Client only
{
...
class FNetworkPredictionData_Client* UVRMovementComponent::GetPredictionData_Client() const
{
	check(PawnOwner != nullptr);

	if (!ClientPredictionData)
	{
		UVRMovementComponent* MutableThis = const_cast<UVRMovementComponent*>(this);
...
void UVRMovementComponent::FSavedMove_My::Clear()
{
	Super::Clear();
...
uint8 UVRMovementComponent::FSavedMove_My::GetCompressedFlags() const
{
...
void UVRMovementComponent::FSavedMove_My::SetMoveFor(ACharacter* Character, float InDeltaTime,
                                                               ...
                                                               ClientData)
{
	...
	UVRMovementComponent* CharacterMovement = Cast<UVRMovementComponent>(Character->GetCharacterMovement());
...
void UVRMovementComponent::FSavedMove_My::PrepMoveFor(ACharacter* Character)
{
...
UVRMovementComponent::FNetworkPredictionData_Client_My::FNetworkPredictionData_Client_My(
	const UCharacterMovementComponent& ClientMovement)
	: Super(ClientMovement)
{
}

FSavedMovePtr UVRMovementComponent::FNetworkPredictionData_Client_My::AllocateNewMove()
{
	return MakeShared<FSavedMove_My>();
}

void UVRMovementComponent::Server_SetAllowedGait_Implementation(const EALSGait NewAllowedGait)
{
	AllowedGait = NewAllowedGait;
}

float UVRMovementComponent::GetMappedSpeed() const
{
...
void UVRMovementComponent::SetMovementSettings(FALSMovementSettings NewMovementSettings)
{
...
void UVRMovementComponent::SetAllowedGait(EALSGait NewAllowedGait)
{
