...
AVRPlayerCameraManager::AVRPlayerCameraManager()
{
...
void AVRPlayerCameraManager::OnPossess(AVRPlayer* NewCharacter)
{
...
float AVRPlayerCameraManager::GetCameraBehaviorParam(FName CurveName) const
{
...
void AVRPlayerCameraManager::UpdateViewTargetInternal(FTViewTarget& OutVT, float DeltaTime)
{
	...
	if (OutVT.Target)
	{
		...
		if (OutVT.Target->IsA<AVRBasePlayer>())
		{
...
FVector AVRPlayerCameraManager::CalculateAxisIndependentLag(FVector CurrentLocation, FVector TargetLocation,
...
bool AVRPlayerCameraManager::CustomCameraBehavior(float DeltaTime, FVector& Location, FRotator& Rotation, float& FOV)
{
